---
title: "Effectiveness of European Regional Development Funds in reducing unemployment: a temporal analysis"
author: "Diego Paroli"
date: "`r Sys.Date()`"
output:
  rmdformats::robobook
...
This document contains the code used for the empirical analysis conducted in my final thesis for the Master in Computational Social Sciences.
...
# Difference between NUTS code in ependiture data and official 2013 NUTS codes
setdiff(nuts_funds, nuts_2013)
```

There are some differencies. As mentioned here: [Historic EU investment](https://cohesiondata.ec.europa.eu/stories/s/Historic-EU-payments-by-region-1988-2022/47md-x4nq), "the 2013 version of the NUTS codes is used predominantly (in the dataset)", but hence, not always...

The ones above are some exceptions which I must fix for. They are mainly NUTS 2010 codes which have not been updated. I will do this below here for the regions which have not been subject to boundary changes.
...
# I assume this is a mistake by the data provider, but it was always meant to use 
# the NUTS 2013 version SI03 and SI04
funds |> 
  filter(Country == "SI") |> 
...
# Checking now again which NUTS codes are still not in NUTS 2013
nuts_funds <- unique(funds$NUTS2_ID)
setdiff(nuts_funds, nuts_2013)
```

The remaining regions had a boundary shift and they must be deleted as I cannot use their data (data cannot be linked properly to a NUTS-2013 code).
...
Now I must transform from NUTS2013 codes to NUTS2021 codes as my dependent variable and covariates are going to be in NUTS2021 code.

I checked all the changes that there have been in the NUTS codes from [Eurostat NUTS webpage](https://ec.europa.eu/eurostat/web/nuts/history). There are boundary changes which cannot be handled or there are other small changes (sometimes even just code changes) that can instead be handled.

```{r}
# Getting the official 2021 NUTS codes
...
# See which codes in my dataframe are not the same of their 2021 versions
...
Now all the remaining regions are in NUTS 2021 code. The regions for which it was impossible to retrieve NUTS 2021 data have been eliminated.

I'll now check that each region has one name only.
...
I'll look at Greece first:

```{r}
dup <- funds |> filter(NUTS2_ID == "EL51")
unique(dup$NUTS2_ID)
unique(dup$NUTS2_name)
# Just minor grammatical differences
...
```{r}
# There are some regions that have more than 34 observations 
# 34 observation should be the maximum given that the data spans from 1989 to 2022)
...
Only the ones that needed help before are duplicates. I will now check how to fix their duplicates.

```{r}
# No need to do anything special because there is no overlapping of data.
# Just need to collapse all the data in one row
funds |> 
...
Unfortunately there is no Ireland because all its NUTS2 codes had boundary changes and thus have been deleted. There is also no Lithuania because its only NUTS 2 region has been deleted due to being split it up (thus having a boundary change).
...
As some regions are not there even in years where they should be (showed by the fluctuating number of regions/countries per year) I need to create synthetic data for them (I assume that if they are not there is because they did not receive any money that year).

Each code chunk represents a period to be fixed.
...
Looking at the '95 enlargement now:
...
missing_1993
# These regions should not be there as they were not in the EU before 1995
...
# I will handle it the same way setting all expenditure for the new regions to 2004
# Getting the data to be modified
df <- funds |>
...
Even though Brexit happened in 2020 UK regions were still allowed to continue access to funds allocated for 2014-2020 budget (source: [UK govt](https://www.gov.uk/guidance/eu-funded-programmes-under-the-withdrawal-agreement)).
...
Next I must set as NAs all the observations that are outside of the expenditure timeframe and set to 0s in the case that there is no expenditure, but observations are within the expenditure timeframe.
...
```{r}
available_variables <- ARDECO::ardeco_get_variable_list()
head(available_variables)
```

Focusing now on unemployment, the dependent variable.
...
# It looks like unemployment data is present for all my regions in the expenditure df
# Only 1 regions is missing
...
# It looks like gdp data is present for all the regions
...
# It looks like population data is present for all the regions
...
# It looks like education data is present for all regions
...
# It looks like investment data is present for all regions
...
```{r, message=FALSE, warning=FALSE}
cat14_20 <- read_csv("data/cat14_20.csv")
...
This categorization uses NUTS 2006 vs NUTS 2021 used in my dataset. Therefore I must fix this.

```{r}
# 2006 Regions that are not present in the dataset due to NUTS discrepancies
...
The only regions that have been left out (present in the classification, but not in the main df) are part of the regions which I have previously deleted above because of boundary shifts. Thus everything is fine.

I join the classification and make sure it appears for the whole budget period they refer to + n years of implementation.
...
```{r, message=FALSE, warning=FALSE}
cat07_13 <- read_csv("data/cat07_13.csv")
...
# Regions that are not present in the dataset due to NUTS discrepancies
...
Again, the only regions that have been left out (present in the classification, but not in tha main df) are part of the regions which I have previously deleted above because of boundary shifts. Thus everything is fine.
...
```{r, message=FALSE, warning=FALSE}
cat00_06 <- read_csv("data/cat00_06.csv")
...
Again everything is fine and I can join the classification:
...
data <- data |> 
  select(-`expenditure_1989-1993_budget`, -`expenditure_1994-1999_budget`)
...
```{r}
# Step 1: Set the columns I want to impute
...
pred_matrix[c("Country", "NUTS2_name", 
              "expenditure_2000-2006_budget",
              "expenditure_2007-2013_budget", "expenditure_2014-2020_budget",
              "poor00_06", "poor07_13", "poor14_20"), ] <- 0
...
## EDA: Total expenditure per year
...
# ggsave("tot_exp_barplot.png", plot = p, width = 8, height = 4.5, dpi = 300)

p
```

## EDA: Table per country

```{r}
country_summary <- data |> 
  mutate(unemployment = ((unemployment*1000)/population)*1000) |> 
  mutate(investment = (investment*1000000/population)) |> 
  mutate(tot_exp = rowSums(across(starts_with("expenditure")), na.rm = TRUE)) |>
  group_by(Country) |> 
  summarise(
    n_regions = n_distinct(NUTS2_ID),
    sum_exp = sum(tot_exp, na.rm = TRUE)/1000000,
...
table_country <- country_summary |> 
  gt() |> 
  tab_header(
    title = "Average Economic Indicators by Country",
    subtitle = "Aggregated over the full 2000–2022 period or since EU accession"
  ) |> 
  cols_label(
    Country = "Country",
    n_regions = "N. of Regions",
    sum_exp = "Total ERDF Expenditure",
  ...
  fmt_number(
    columns = c(avg_gdp, avg_investment, sum_exp),
    decimals = 0,
    use_seps = TRUE
  ) |> 
  fmt_number(
    columns = c(avg_education, avg_unemployment),
    decimals = 1,
    use_seps = TRUE
  ) |> 
  cols_align(
    align = "center",
    columns = c(avg_unemployment, avg_gdp, avg_investment, avg_education)
  ) |> 
  cols_align(
    align = "right",
    columns = c(n_regions, sum_exp)
  ) |>
  tab_spanner(
    label = "Economic Indicators (Country Averages)",
    columns = c(avg_unemployment, avg_gdp, avg_investment, avg_education)
  ) |> 
  tab_source_note("Note: Expenditure in millions €; GDP and investment per capita in €; number of unemployed per 1000 people; % of population aged 25–64 with tertiary education."
  ) |> 
  tab_options(
...
# gtsave(table_country, filename = "country_descriptive.png")
...
data |> filter(is.na(unemployment)) |> select(NUTS2_ID, Year, unemployment)
...
# Drop FI20 always because of no data
data <- data |> 
...
main_plot <- ggplot(main_map) +
  ...
  labels = c("Non-Least Developed", "Least Developed", "No data", "Non-EU"),
  guide = "legend") + 
  theme_void() +
...
### Maps for 2007-2013 budget period
...
### Maps for 2014-2020 budget period
...
table_gt <- final_summary |> 
  ...
  fmt_number(
    ...
    use_seps = TRUE) |> 
  fmt_number(
    columns = c(avg_expenditure, avg_unemployment, avg_education),
    decimals = 1,
    use_seps = TRUE) |>
  cols_align(
    align = "center",
  ...
  tab_source_note("Note: Expenditure, GDP and investment per capita in €; number of unemployed per 1000 people; % of population aged 25–64 with tertiary education.") |> 
  tab_options(
    heading.background.color = "#f8f9fa",
    column_labels.background.color = "#e9ecef",
...
p <- ggplot(coef_table, aes(x = as.numeric(year), y = estimate, color = term)) +
  ...
  geom_point(size = 2) +
  scale_x_continuous(breaks = seq(from = min(as.numeric(coef_table$year)), 
                                  to = max(as.numeric(coef_table$year)), 
                                  by = 2)) +
  labs(
    title = "Estimated Effect of ERDF Expenditure on Unemployment in Poor Regions",
    subtitle = "Interaction Term by Budget Period. Insiginficant Coefficients set to 0.",
    x = "Year",
    y = "Coefficients",
    color = "Budget Period"
  ) +
...
# ggsave("expenditure_coefficients.png", plot = p, width = 8, height = 4, dpi = 500)
