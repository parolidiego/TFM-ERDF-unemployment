...
modelsummary(
  ...
  stars = TRUE,
  statistic = NULL,
  coef_map = c("lag_unemployment", "gdp", "investment", "education",
               "expenditure_1994-1999_budget", "expenditure_2000-2006_budget",
               "expenditure_2007-2013_budget", "expenditure_2014-2020_budget",
               "poor00_06", "poor07_13", "poor14_20",
               "expenditure_2000-2006_budget:poor00_06",
...
Check total expenditure  -> Can be deleted

```{r}
data_growth <- data %>%
  mutate(tot_exp = rowSums(select(., starts_with("expenditure")), na.rm = TRUE)) |>
  select(-c("expenditure_2000-2006_budget",
            "expenditure_2007-2013_budget", 
            "expenditure_2014-2020_budget")) |> 
  mutate(poor00_06 = ifelse(Year > 2006, NA, poor00_06),
         poor07_13 = ifelse(Year > 2013, NA, poor07_13)) |> 
  group_by(NUTS2_ID) |> 
  mutate(
    unemployment = (unemployment - lag(unemployment)) / lag(unemployment) * 100,
    gdp = (gdp - lag(gdp, 3)) / lag(gdp, 3) * 100,
    investment = (investment - lag(investment, 3)) / lag(investment, 3) * 100,
    education = (education - lag(education, 3)) / lag(education, 3) * 100,
    tot_exp = (log(tot_exp + 1e-6) - log(lag(tot_exp + 1e-6, 3))) * 100) |> 
  ungroup()
...
```{r}
regression_growth <- map(list_data_growth_cleaned, function(df) {
  exp_vars <- names(df)[startsWith(names(df), "tot_exp")]
  poor_vars <- names(df)[startsWith(names(df), "poor")]

  # Main effects
  all_vars <- c(exp_vars, poor_vars)
  ...
  formula <- as.formula(paste(
    "unemployment ~ . +",
    paste(c(interaction_terms, all_vars), collapse = " + ")
  ))
...
modelsummary(
  ...
  statistic = NULL,
  coef_map = c("gdp", "investment", "education",
               "tot_exp", "poor00_06", "poor07_13", "poor14_20",
               "poor00_06:tot_exp", "poor07_13:tot_exp", "poor14_20:tot_exp"),
  gof_map = list(
    list("raw" = "nobs", "clean" = "N, Obs", "fmt" = 0),
    list("raw" = "r.squared", "clean" = "R2", "fmt" = 2),
    list("raw" = "adj.r.squared", "clean" = "Adj. R2", "fmt" = 2)),
...
```{r}
mean(map_dbl(regression_growth, ~ summary(.)$r.squared))
count_significant(regression_growth, "poor00_06:tot_exp")
count_significant(regression_growth, "poor07_13:tot_exp")
count_significant(regression_growth, "poor14_20:tot_exp")
```